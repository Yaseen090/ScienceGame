...
public class PlayerVsComputerUI extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        primaryStage.setTitle("Player vs. Computer Mode");
        ...
        // Create a label
        Text titleText = new Text("Player vs. Computer Mode");
        ...
        submitButton.setOnAction(event -> {
            if (validateMove(gameBoard)) {
                ...
                //Computer Player
                String randomWord=dictionary.getWSet().stream().toList().get(random.nextInt(dictionary.getWSet().size()));
                int x=random.nextInt(15-randomWord.length());
                int y=random.nextInt(15-randomWord.length());
                while(gameBoard.getTile(x,y).getLetter()!='\0')
                {
                    x=random.nextInt(15-randomWord.length());
                    y=random.nextInt(15-randomWord.length());
                }
                for(int i=0;i<randomWord.length();i++)
                {
                    gameBoard.addTile(new Tile(randomWord.charAt(i),i+x,y));
                }
                ...
                computerScore+=calculateScore(gameBoard.getTempTilesList());
                primaryStage.show();
                if(totaltiles>=10)
                {
