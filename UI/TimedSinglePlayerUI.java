...
public class TimedSinglePlayerUI extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        primaryStage.setTitle("Timed Single Player Mode");

        // Create a label
        Text titleText = new Text("Timed Single Player Mode");
        ...
        AtomicInteger timeSeconds = new AtomicInteger(60); // Set the timer for 60 seconds
        Timeline timeline = new Timeline();
        KeyFrame frame = new KeyFrame(Duration.seconds(1), e -> {
            int curr=timeSeconds.decrementAndGet();
        // Update the label
        ...
        if (curr <= 0) {
            ...
            alert.setHeaderText("Time Finished!");
...
//                gameBoard.updateCurrentTurnTiles(combinedList);
                primaryStage.show();
                if(totaltiles>=50)
                {
                    Alert alert = new Alert(AlertType.INFORMATION);
        ...
        layout.getChildren().addAll(titleText,scoreLayout,timerLabel,gameLayout,submitButton);
